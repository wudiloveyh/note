# 主机信息收集

## 主机信息收集需要收集哪些内容

### 基本主机信息

判断系统是否存在可以利用的漏洞、补丁确实、是否可以提权

- 操作系统版本

  ```diff
  - windows
  systeminfo | findstr OS
  ver
  reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion" /v ProductName
  - linux
  cat /etc/os-release
  ```

- 补丁情况

  ```diff
  - windows
  systeminfo | findstr KB
  wmic qfe list brief
  - linux 
  apt list --installed
  rpm -qa --last
  ```

- 系统架构

  ```diff
  - windows
  systeminfo
  echo %PROCESSOR_ARCHITECTURE%
  wmic os get osarchitecture
  - linux
  uname -m
  # 查看cpu架构
  lscpu
  # 查看内核架构
  uname -a 
  ```

- 主机名

  ```diff
  + windows和linux都可以执行hostname查看
  hostname
  - winodws
  echo %COMPUTERNAME%
  - linux详细
  hostnamectl
  ```

- 系统运行时间

  ```diff
  - windows
  systeminfo | findstr "系统启动时间"
  net statistics workstation
  - linux
  uptime
  ```

- 是否是虚拟机运行

  ```diff
  - windows
  wmic computersystem get model, manufacturer
  (Get-WmiObject Win32_ComputerSystem).Manufacturer
  + 如果输出下面的表格的内容就大概是虚拟机了
  ```

  | 虚拟机类型 | 典型 Manufacturer / Model               |
  | ---------- | --------------------------------------- |
  | VMware     | VMware, Inc. / VMware Virtual Platform  |
  | VirtualBox | innotek GmbH / VirtualBox               |
  | Hyper-V    | Microsoft Corporation / Virtual Machine |
  | KVM / QEMU | QEMU / Standard PC (Q35 + ICH9)         |
  | Xen        | Xen / HVM domU                          |

  ![image-20251115160138583](https://cdn.jsdelivr.net/gh/wudiloveyh/my-image-bucket@main/images/20251115160138658.png)

  真实物理机会显示：

  ```diff
  LENOVO
  DELL
  HP
  ASUS
  Acer
  ```

  ![image-20251115160145566](https://cdn.jsdelivr.net/gh/wudiloveyh/my-image-bucket@main/images/20251115160145631.png)

  也可以查看mac地址判断：

  ```diff
  getmac
  ```

  linux:

  ```diff
  egrep -i 'hypervisor|vmx|svm' /proc/cpuinfo
  ```

  - 如果出现 `hypervisor` 字段 → 一般说明运行在虚拟机中
  - `vmx` → Intel VT-x（硬件虚拟化）
  - `svm` → AMD-V（硬件虚拟化）

- 是否存在域环境

  ```diff
  systeminfo | findstr "域"
  systeminfo | findstr "Domain"
  ```

  ![image-20251115160738146](https://cdn.jsdelivr.net/gh/wudiloveyh/my-image-bucket@main/images/20251115160738201.png)

### 用户和权限信息

判断能否提权、横向移动是否有凭据、是否有特权用户

#### 本地用户信息

- 本地用户列表

  ```diff
  - windows
  net user
  - linux 
  cat /etc/passwd
  + 列出当前登录用户
  who
  + 列出所有登录过的用户
  last
  ```

- 用户SID

  ```diff
  - windows
  wmic useraccount get name,sid
  Get-WmiObject Win32_UserAccount -Filter "LocalAccount=True" | Select Name,SID
  - linux
  + Linux 没有 Windows SID，但 UID/GID 起类似作用。
  id username
  ```

- 用户是否禁用/启用

  ```diff
  - windows
  wmic useraccount get name,disabled
  Get-WmiObject Win32_UserAccount -Filter "LocalAccount=True" | Select Name,Disabled
  - linux
  + 有实际密码就是启用，没有的话是***，然后带有两个感叹号
  sudo cat /etc/shadow | grep username
  ```

- 用户组、管理员组成员

  ```diff
  - windows
  net user username
  net localgroup Administrators
  - linux 
  groups username
  + 输出：username：主组，后续的是附加组
  cat /etc/group
  getent group sudo
  getent group wheel
  ```

#### 权限相关

- 当前用户权限

  ```diff
  - windows
  whoami /groups
  - linux 
  sudo -l
  ```

### 凭证相关信息（相当重要）

Windows 本地账户的密码哈希存储在 **SAM 文件** 中（Security Accounts Manager）。

路径：

```diff
C:\Windows\System32\config\SAM
C:\Windows\System32\config\SYSTEM
```

- SAM 文件存储账户和 LM/NTLM 哈希

- SYSTEM 文件用于解密 SAM 中的哈希
- 一般提取凭证需要管理员权限

横向移动、提权的关键部分

- 本地密码哈希

  用工具导出

  mimikatz：

  项目地址：

  https://github.com/gentilkiwi/mimikatz.git

  ```diff
  - 基本命令，要用管理员权限运行
  log mimi.log
  privilege::debug
  sekurlsa::logonpasswords
  exit
  + 查看mimi.log就可以看到抓取的哈希
  ```

  ![image-20251115170306010](https://cdn.jsdelivr.net/gh/wudiloveyh/my-image-bucket@main/images/20251115170306075.png)

- 内存中明文密码

  和上面的操作一样，这里用另一种方法，不直接在目标主机运行mimikatz，先用procdump把LSASS进程的完整内存Dump下来保存到一个文件

  procdump下载地址：

  https://download.sysinternals.com/files/Procdump.zip

  ```bash
  procdump -ma lsass.exe lsass.dmp
  # -ma👉Full dump，包含进程的 完整内存，堆栈、堆、句柄等都保存下来
  mimikatz "sekurlsa::minidump lsass.dmp" "sekurlsa::logonpasswords"
  ```

  ![image-20251115175632702](https://cdn.jsdelivr.net/gh/wudiloveyh/my-image-bucket@main/images/20251115175632767.png)

  windows7或者其他系统如果运行procdump失败，可以尝试老版本

- 浏览器保存的密码

  打开浏览器查看，一般需要输入密码打开对应的保险箱

- 软件保存的密码

  某些软件保存的密码

- ssh私钥

  本质是文件

  找到存放的位置

  可能存在的位置：

  ```diff
  ~/.ssh/id_rsa
  ~/.ssh/id_ecdsa
  ~/.ssh/id_ed25519
  ~/.ssh/id_dsa
  /root/.ssh/
  /home/*/.ssh/
  /etc/ssh/ssh_host_rsa_key（主机私钥）
  ```

  找到后可以尝试登录：

  ```bash
  ssh -i id_rsa user@target
  ```

  如果能够成功登录的话就存在凭证泄露风险

  ![image-20251115192335788](https://cdn.jsdelivr.net/gh/wudiloveyh/my-image-bucket@main/images/20251115192336115.png)

  上图演示了把私钥的内容复制到1.txt中，利用这个私钥文件成功免密登录kali

- web应用配置文件密码

  php：

  | 框架      | 配置文件路径                         |
  | --------- | ------------------------------------ |
  | WordPress | `/var/www/html/wp-config.php`        |
  | ThinkPHP  | `application/database.php` 或 `.env` |
  | Laravel   | `.env`                               |
  | Discuz    | `config/config_global.php`           |

  java

  ```bash
  application.properties
  application.yml
  /opt/app/
  /usr/local/app/
  /usr/share/tomcat/webapps/xxx/
  ```

- DPAPI存储的凭证

  **DPAPI（Data Protection API）** 是 Windows 内置的数据加密体系，用于把敏感数据加密并存储在系统中。

  查看DPAPI目录：

  用户级

  ```bash
  dir C:\Users\<user>\AppData\Roaming\Microsoft\Protect\
  ```

  系统级：

  ```bash
  dir C:\Windows\System32\Microsoft\Protect\
  ```

### 网络和通信信息

用来判断横向移动是否可行、路径和网络结构

##### IP和网络信息

- IP地址、子网掩码、网关

  ```diff
  - windows
  ipconfig /all
  route print
  - linux
  ifconfig
  ip a 
  route -n
  ```

- DNS服务器

  ```diff
  - windows
  ipconfig /all
  - linux
  cat /etc/resolv.conf
  ```

- hosts文件内容

  ```diff
  - windows
  type C:\Windows\System32\drivers\etc\hosts
  - linux
  cat /etc/hosts
  nl /etc/hosts
  ```

- 多网卡情况

  对每个网段进行存活探测

  ```bash
  for /L %i in (1,1,254) do @ping -n 1 -w 1 10.1.1.%i | find "TTL="
  ```

  

##### 路由信息

```bash
# windows
route print
# linux
route -n 
```

##### 端口/服务

- 当前主机开放的端口
- 监听的服务
- 端口可达性检查（说不定外部无法访问）

##### 连接情况

- 当前TCP/UDP连接
- 正在连接的内部系统（MSSQL）

### 已安装的软件和服务

用于判断是否存在可利用的漏洞组件或弱点

##### 安装的软件列表

- windows：

  ```bash
  wmic product
  ```

- linux:

  ```bash
  dpkg -l
  rpm -qa
  ```

##### 中间件服务

- Tomcat/Nginx/Apache
- PHP/Java/Python版本
- 数据库版本
- 远程服务（VNC）

##### 自启动程序

- windows启动项、服务自启动
- linux systemd、init、crontab

### 文件和目录

常见的敏感文件、可利用文件、后门文件等

- 可疑的脚本（.bat/.ps1/.sh）
- 本地敏感文件（密码本、配置文件）
- Web路径中的配置文件
- 日志文件（可用来找凭据）

#### 任务计划/Cron/自启动

提权和持久化常见点

- windows计划任务
- linux crontab/systemd timer
- 可以持久化点

#### 安全防护和日志

用来判断如何绕过防护和规避检测

- windows Defender状态
- 杀软
- EDR是否开启
- 防火墙策略
- 审计策略情况

#### 系统漏洞和弱点检测

用于判断可利用点

- SUID文件
- 不安全的服务权限
- 可写的服务路径/DLL劫持点
- 未修补的高危补丁
- UAC状态
- 共享目录权限问题

#### 容器/虚拟化/云环境标识（如果有）

用来判断是否能逃逸到宿主机

- 容器运行环境检测（Docker、LXC）
- 虚拟化信息（VMware、Hyper-V）
- 云平台元数据检查（AWS/Aliyun）

## 工具

### peass-ng

项目地址

https://github.com/peass-ng/PEASS-ng.git

下载列表中有很多版本，其中linux的版本：

![image-20251115141120932](https://cdn.jsdelivr.net/gh/wudiloveyh/my-image-bucket@main/images/20251115141120984.png)

分别是不同架构的版本，没有后缀的是编译好的版本，sh是脚本版本，兼容性比较好，其中的fat代表功能完整，small代表只有主要的功能

对于windows：
![image-20251115141308730](https://cdn.jsdelivr.net/gh/wudiloveyh/my-image-bucket@main/images/20251115141308777.png)

其中bat是脚本版本，兼容性比较好，其中的any是完整功能版本，后缀ofs代表代码做了混淆，可以绕过一些安全软件

##### 使用方式

linux：

```bash
# 上传linpeas_fat.sh到目标主机
# 赋予执行权限
chmod +x linpeas_fat.sh
./linpeas_fat.sh
# 上面的命令会吧收集的内容打印在终端
# 需要打印在终端的同时保存结果文件
./linpeas_fat.sh -a | tee result.log
# 这个fat运行时间比较长，耐心等待就好了
# 把这个输出文件下载到本地电脑之后用vscode打开，记得下载iliazeus的ansi color插件，并运行：ANSI Text: Open Preview
# 输出无颜色结果
linpeas_fat.sh --no-color | tee result.log
```

![image-20251115145804813](https://cdn.jsdelivr.net/gh/wudiloveyh/my-image-bucket@main/images/20251115145805037.png)

windows：

```bash
# 命令行运行
winpeass.exe
# 输出结果到文件
winpeass.exe > result.txt
# 在一些比较垃圾的机器上如果提示说缺少.NET框架的话可以运行.bat版本的工具
# 等待工具运行的log结果给vscode看就可以了
# 输出无颜色结果
winpeas.bat --no-color
```

![image-20251115151203436](https://cdn.jsdelivr.net/gh/wudiloveyh/my-image-bucket@main/images/20251115151203709.png)



